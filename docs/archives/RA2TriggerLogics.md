...
但随着触发链越来越长，这种层层缩进的形式也会因为不同结果的穿插变得混乱不堪。除此之外，这种表示也很难解释 *Q54 禁止触发* 这条行为。由于本人水平有限，这两点缺陷就权当是留给有兴趣的读者一道思考题吧。
...
### 2.4 线性多分支选择结构
在触发设计中，有的时候还会希望在某一个节点，根据不同的情况分别做出不同的反应，这就涉及到了选择。选择结构是创建分支流程的重要组成部分，可用于实现**条件发生变化时流程也随之发生改变**的效果。当大量的选择结构简单串联之后，实际上就构成了线性多分支结构^3^。
...
而在涉及到多个条件时，不同条件之间的关系也会影响选择结构的实际效果。具体来说，是互斥与否的关系。
...
像这样将条件映射到局部变量，用变量代为影响执行流的“门电路”思想，接下来会多次体现。
...
在开始之前，不妨先捋一捋这个条件组的逻辑。这实际上是个必要不充分条件：要保证*完成流程*，必需满足以下两个分条件：
