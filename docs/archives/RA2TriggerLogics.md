...
尽管触发系统并没有指出 $\neg p$ 应该如何处理，但从上图也不难看出，`else`实际上就是`if not`。这种反向考虑条件的思路叫做**反转 *if***。在实际应用中，由于在触发里实现双分支的难度比较大，一般是借助局部变量辅助，把它变换为下面要讨论的复合条件实现；或者干脆放弃另一个分支，只考虑条件（不）满足时的情形。既然涉及到局部变量，详细的思路就留待下一章再详细讨论了。
...
由于触发的 $p \to q$ 语义无法通过逻辑上互斥实现选择，因此相比泾渭分明的`else if`，触发更倾向物理实现这个互斥：一旦某一分支成功触发，就需要**尽快阻止触发其他分支**。实践中通常会考虑 *54 禁止触发*、摧毁选择器等方式，让用户**来不及**进入另外的分支。
...
::: tip 将双分支选择转化为基于变量值的多分支选择
基于 3.2 介绍的局部变量读、写触发，以及本节对“反转 if”的实例分析，
:::
