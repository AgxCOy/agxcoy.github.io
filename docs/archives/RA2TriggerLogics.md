...
::: note 逻辑与实现
必须指出的一点是：*逻辑*与*实现*并不等同。逻辑关心客观事物的**本质、规律**，实现则关心符合这个规律的**解决方案**。
:::
...
> 但考虑到 Python 的很多特性存在理解门槛，本章将只着眼于`if`等通用范式。
...
至于事件`condition`和行为`do_something()`，已知它们可以不止一条。那么多条件和多结果是如何串起来的呢？
...
while elapsedTime(6):  # 每隔 6 秒
  ...
  reinforcement("01014514")  # 7 - 援军（小队）
```

### 2.4 选择结构
在触发设计中，有的时候还会希望在某一个节点，根据不同的情况分别做出不同的反应，这就涉及到了选择。选择结构是创建分支流程的重要组成部分，可用于实现**条件发生变化时流程也随之发生改变**的效果^3^。

无论在生活、数学当中，流程图还是编程，选择结构都有“要不然”、“否则”等逻辑指示词，往往容易理解。然而遗憾的是，红警 2 $p \to q$ 的触发语义并未对 ***p* 不满足**的情形采取任何措施，届时要讲清楚选择结构，就不得不先回归生活、数学当中去探讨「选择」的本质了。
...
单分支只关心**条件满足时**要做什么——与触发的工作逻辑很像。这种单分支结构可能会执行图中的处理程序，也可能无事发生。比如这样一个生活例子：*要是下雨了，就去阳台收衣服*。那假如没下雨呢？不知道，没说。  
双分支则同时关心这个条件**不满足时**做什么，也就是无论如何都会执行**其中一个**处理程序。还是之前那个例子：*待会要是下雨就收衣服，没下雨就把中药也拿去晒*。那无论下不下雨，你都免不了要跑一趟。

当然前面说过，触发并没有类似*否则*的设计，因此双分支实现起来需要利用程序重构的一些方法，比如说反转`if`。具体思路还请参见[逻辑非运算](#_3-3-2-非运算——逆向思维)。
...
如果没有加上那个「反之」，可以预见有这么一个数`251`，先过第一条分支变成`-251`，随后又过第二条分支，变成`-15,813,251`。而一旦加上了这个「反之」，它就又变成了互斥：要么 $0 \lt x \le 256$ 进入第一支，否则就 $256 \lt x \le 512$ 进入第二支。对于 251 这种漏网之鱼，容易推出它只会经过第一支的处理。

由于触发的 $p \to q$ 语义无法通过逻辑上互斥实现选择，因此相比泾渭分明的`else if`，触发更倾向物理实现这个互斥：一旦某一分支成功触发，就需要**尽快阻止触发其他分支**。实践中通常会考虑*禁止触发*、摧毁选择器等方式，让用户**来不及**进入另外的分支。
...
那么当一个油田*被摧毁*时，首先想到余量`remaining`减一。*如果这个油田先前是玩家持有*，那就再对`player_owns`减一。简单的允许触发即可：

```python
if objDestroyed(OilA):
  remaining -= 1  # 501 编辑变量 (Phobos)
...
### 3.1 案例实现思路
「运输船找妈妈」本质上就是让运载乘客的载具，**打哪来能回哪去**。观察一下 YR S01：时空转移的相关演出，易得流程如下：
...
于是考虑设`apc unload`变量初始为 0，然后在运输船卸出乘客之后*设置局部变量*，触发得知*局部变量被设置*了，就建一个空船小队把它拉走。
...
- 事件 36：局部变量被设定（值为 1），即当`x = 1`时
- 事件 37：局部变量被清除（0），即当`x = 0`时

* 行为 56：设置局部变量（值为 1），即令`x = 1`
* 行为 57：清除局部变量（值），即令`x = 0`
...
if apc_unload == 1:
  create_team("LCRFBack")  # 4 - 建立小队
```

触发就是通过对局部变量值的变动，来实现一些较为复杂的逻辑判断。并且随着`int32`扩展局部变量的出现，触发的判断也不再局限于 0 和 1 的「左手倒右手」，而是与科技类型、超级武器、随机数等联系了起来，实现更加精细的随机机制和流程控制。

### 3.3 或、非逻辑运算
前面说过，触发条件以且仅以**逻辑且**相连。已知的触发实践也表明，多条件不可能在仅满足其中一个的情况下执行触发；前面 2.4 一节也指出触发没有`else`设计，也就莫得*非*逻辑。那是否说明，触发就是做不到逻辑完备，如同早期面对平台限制一样无解呢？非也。
