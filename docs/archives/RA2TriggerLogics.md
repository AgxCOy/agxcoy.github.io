...
Zero Fanker 认为，标签是一种传递事件的「管道」，向触发传递关联对象的状态^4^。一旦触发成功接收到这个状态，**与管道断开了连接**，这个管道也就消亡。从这个角度出发，也可以这么说：那些未与尚存管道相连的触发，在游戏中全程空转；在用户（地图师和玩家）看来，就像是从未存在过。 -->
...
顺序结构是最简单、最基本、最符合思维直觉的结构。在线性系统中，**按时间先后执行的流程**我们认为就是顺序结构^4^。以被广为改编的「脑死」为例，它的任务流程具有很典型的顺序性：首先需要建立一座~~锅盖~~苏军雷达，然后打掉最后一座心灵控制器，最后秋风扫落叶。
...
上述代码段的排布与前面的案例一致，$t_1$ 是排在 $t_0$ 后面的，这样 $t_1$ 必定会等 $t_0$ 先触发；除此之外，$t_1$ 还往右缩进，与 $t_0$ 的那两条结果对齐，意味着 $t_0$ 不触发时，绝对不会触发 $t_1$，以此避免独立触发和触发链的歧义。但随着触发链越来越长，这种层层缩进的形式最终也会变得难以辨认。

大部分任务的设计大体上就是依靠触发链形成分流程，再将各个分流程链接起来形成顺序结构的战役关卡。
...
### 2.4 选择结构
在触发设计中，有的时候还会希望在某一个节点，根据不同的情况分别做出不同的反应。这就涉及到了选择。选择结构是创建分支流程的重要组成部分，可用于实现**条件发生变化时流程也随之发生改变**的效果^4^。

::: note 为什么最后才讨论选择结构
前面顺序结构、循环结构尚且有简单易用的方法，也容易理解。但很遗憾，红警 2 $p \to q$ 的触发语义并未对 ***p* 不满足**的情形采取任何措施，因此对于选择结构的讨论可能需要花一些篇幅。
:::

::: warning 施工暂停（或中止）
我……有些累了。已经够了。果然菜逼什么都做不到，又喜欢胡闹。
:::
...
### 3.3 或、非逻辑运算
前面说过，触发条件以且仅以**逻辑且**相连。已知的触发实践也表明，不可能在仅有一个条件满足的情况下执行触发；前面 2.4 一节也指出触发没有`else`设计，也就莫得*非*逻辑。那是否说明，触发就是做不到逻辑完备，如同早期面对平台限制一样无解呢？非也。
