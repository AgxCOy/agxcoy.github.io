...
### 2.4 选择结构
在触发设计中，有的时候还会希望在某一个节点，根据不同的情况分别做出不同的反应。这就涉及到了选择。选择结构是创建分支流程的重要组成部分，可用于实现**条件发生变化时流程也随之发生改变**的效果^3^。然而遗憾的是，红警 2 $p \to q$ 的触发语义并未对 ***p* 不满足**的情形采取任何措施。为了更好地理解接下来讨论的选择结构触发实现，有必要先从自然语言入手讲清楚「选择」的本质。

![流程图中的“选择结构”](https://image.woshipm.com/wp-files/2017/08/8Ynwb53uWeo9QMHb7Xi5.png =40%x40%)

自然语言中的选择大体可以抽象为上述两种流程图：互斥（右）与非互斥（左）。**互斥**是说，这组条件分支**不可能**同时满足，因此必然只会运行**其中一个**处理程序。比如这样一个生活化的例子：待会*如果下雨*，就出去收衣服；*如果没下雨*，就把中药拿出去晒。很显然，要么下雨要么不下雨，二者是互斥的。**非互斥**是指，条件分支中**至少有两条**同时满足。比如说重叠区间：

- 若 $x \in (0, 256]$，则令 $x = -x$；
- （反之？）若 $x \in (250, 512]$，则又令 $x = x^3$。

如果没有加上那个「反之」，可以预见有这么一个数`251`，先过第一条分支变成`-251`，随后又过第二条分支，变成`-15,813,251`。而一旦加上了这个「反之」，它就又变成了互斥：要么 $0 \lt x \le 256$ 进入第一支，不然就 $256 \lt x \le 512$ 进入第二支。对于`251`这种漏网之鱼，容易推出它只会进入第一支。

而由于触发类似`if`单分支的形式，无法通过逻辑上互斥实现选择，因此相比泾渭分明的`if-elif`，触发更倾向物理实现这个互斥：一旦某一分支成功触发，就需要**尽快阻止触发其他分支**。实践中通常会考虑*禁止触发*、摧毁选择器等方式，让用户**来不及**进入另外的分支。  
