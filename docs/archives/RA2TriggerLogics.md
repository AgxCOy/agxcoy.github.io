...
尽管触发的语义并没有指出 $\neg p$ 应该如何处理，但从流程图也不难看出，`else`实际上也就是`if not`：
```python
if 114514 < 13:
  pass
else:  # 显然 114514 > 13（
...
if 114514 >= 13:
  ...
```
这种反向考虑条件的思路叫做*反转 if*。在实际应用中，由于在触发里实现双分支的难度比较大，一般是借助局部变量辅助；或者干脆放弃其中一支，专注于条件为真（或为假）的情形。既然涉及到局部变量，详细的思路就留待下一章再详细讨论了。
